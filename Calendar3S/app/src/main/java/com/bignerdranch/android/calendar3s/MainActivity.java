...
import com.bignerdranch.android.calendar3s.Json.SelectInsertScheduleJson;
import com.bignerdranch.android.calendar3s.Json.SelectMakeGroupJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyGroupJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyGroupTagJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyMessageJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyProfileJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyScheduleJson;
import com.bignerdranch.android.calendar3s.Json.SelectMyTagJson;
...
import com.bignerdranch.android.calendar3s.SQL.DeleteSqlMaster;
import com.bignerdranch.android.calendar3s.SQL.InsertSqlManager;
import com.bignerdranch.android.calendar3s.SQL.SelectSqlMaster;
import com.bignerdranch.android.calendar3s.SQL.UpdateSqlMaster;
...
//                Log.i("msgTEST","자동추가?"+autoAddEventFromNoti.toString()+" sql: "+autoAddEventFromNoti.getSendSQLString());
    ...
    public String SelectInsertSchedule(String message) {
        ...
        sqlMaster.SelectInsertSchedule(message);

        SelectInsertScheduleJson json = new SelectInsertScheduleJson();
    ...
    public void signinDB(String id, String pw) {
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.signinDB(id,pw);

        SelectMyProfileJson jsonMaster = new SelectMyProfileJson();
        jsonMaster.onPostExecute(sqlMaster.runSqlMessage());

        this.user = jsonMaster.getUser();
        if(this.user == null){
            Log.e("SigninDB","로그인 실패!!");
        }
        else this.user.showData();
    }
    public boolean CheckUser(String message){
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.CheckUser(message);

        SelectMyProfileJson json = new SelectMyProfileJson();
        json.onPostExecute(sqlMaster.runSqlMessage());

        UserData userData = json.getUser();
    ...
    public void freshMySchedule() {
        /* DB에서 스케줄 가져오기 */
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.freshMySchedule(user.getData(0));
        SelectMyScheduleJson json = new SelectMyScheduleJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        this.eventDatas = json.getEvents();
        if (this.eventDatas != null)
            for (int i = 0; i < eventDatas.size(); i++) eventDatas.get(i).showData();
    }
    private String selectMyGroupTag(String sql) {
        /* DB에서 스케줄 태그가져오기 */
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.selectMyGroupTag(sql);
        SelectMyGroupTagJson json = new SelectMyGroupTagJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        return json.getResult();
    }
    public void freshTag() {
        /* DB에서 스케줄 태그가져오기 */
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.freshTag(user.getData(0));
        SelectMyTagJson json = new SelectMyTagJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        this.tagDatas = json.getTags();
    }
    public void freshGroup() {
        /* DB에서 내 그룹들 가져오기 */
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.freshGroup(user.getData(0));
        SelectMyGroupJson json = new SelectMyGroupJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        this.groupDatas = json.getGroups();
        if (this.groupDatas != null)
            for (int i = 0; i < groupDatas.size(); i++) groupDatas.get(i).setUserIds_Arr();
    }
    public void freshMessage() {
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.freshMessage(user.getData(0));
        SelectMyMessageJson json = new SelectMyMessageJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        this.messages = json.getMessages();
    }
    private String selectMakeGroup(String id, String newGroupName) {
        SelectSqlMaster sqlMaster = new SelectSqlMaster();
        sqlMaster.selectMakeGroup(id, newGroupName);
        SelectMakeGroupJson json = new SelectMakeGroupJson();
        json.onPostExecute(sqlMaster.runSqlMessage());
        return json.getResult();
    }
    ...
    public void insertScheduleDB(String message) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.insertScheduleDB(message);
    }
    private void joinScheduleDB(String message) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.joinScheduleDB(message);
    }
    public void joinGroupSchedule(String message) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.joinGroupSchedule(message);
    }
    public void InsertTag(String tagname, String color) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.InsertTag(tagname, color);
        freshTag();
    }
    public void InsertGroupTag(String sql) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.InsertGroupTag(sql);
    }
    public void JoinGroup(String sql) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.JoinGroup(sql);
    }
    public void InsertGroup(String sql) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.InsertGroup(sql);
    }
    public void InsertMessage(String sql) {
        InsertSqlManager sqlManager = new InsertSqlManager();
        sqlManager.InsertMessage(sql);
    }

    //DB SQL메소드 - update
    public void UpdateSchedule(String eventId, String eventTitle, String startTimedB, String endTimedB, String location) {
        UpdateSqlMaster sqlMaster = new UpdateSqlMaster();
        sqlMaster.UpdateSchedule(eventId,eventTitle,startTimedB,endTimedB,location);
    }
    public void UpdateScheduleJoin(String eventId, String tagId, String Googleid) {
        UpdateSqlMaster sqlMaster = new UpdateSqlMaster();
        sqlMaster.UpdateScheduleJoin(eventId,tagId,Googleid);
    }

    //DB SQL메소드 - delete
    public void DeleteScheduleJoin(String sid, String Googleid) {
        DeleteSqlMaster sqlMaster = new DeleteSqlMaster();
        sqlMaster.DeleteScheduleJoin(sid,Googleid);
    }
    public void DeleteMySchedule(String sid) {
        DeleteSqlMaster sqlMaster = new DeleteSqlMaster();
        sqlMaster.DeleteMySchedule(sid);
    }
    public void DeleteGroup(String gid) {
        DeleteSqlMaster sqlMaster = new DeleteSqlMaster();
        sqlMaster.DeleteGroup(gid);
    }
    ...
    public void makegroup(String newGroupName, String newGroupIntro) {
        ...
        //그룹에 가입
        String Gid = selectMakeGroup(id, newGroupName);
        sql = "'" + id + "','" + Gid + "'";
        JoinGroup(sql);
